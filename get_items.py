...
def get_items(item_list):
    ...
    for item in item_list:
        ...
        # Price - GEL
        try:
            appartment_info['price_gel'] = page_soup.find("div",{"class":"price"}).find('span',{'class' : 'd-block'})['data-price-gel']
        except:
            pass
        # Price - USD
        appartment_info['price_usd'] = page_soup.find("div",{"class":"price"}).find('span',{'class' : 'd-block'})['data-price-usd']
        # Building Type
        appartment_info['b_type'] = page_soup.find("div",{"class":"main-features"}).find("span",{"class":"pr-1"}).text
        # area 
        appartment_info['area'] = page_soup.find("div",{"class":"_asd"}).find("div",{"class":"space"}).text.replace('Area: ','').replace(' m² ','')
        # description
        try:
            appartment_info['d_original'] = page_soup.find("div",{"class":"description"}).find('p',{"class":"original"}).text.strip()
            appartment_info['d_translated'] = page_soup.find("div",{"class":"description"}).find('p',{"class":"translated"}).text.strip()
        except:
            pass
